"""
hooks.py — Public facade for the hooks subsystem.

This module re-exports every public symbol from the three implementation
sub-modules so callers can always do::

    from claude_code.utils.hooks import execute_pre_tool_hooks, HookResult, ...

Original TypeScript source: utils/hooks.ts (5022 lines).
...
NOTE: Python package-resolution rules mean that when both a ``hooks/``
directory and a ``hooks.py`` file exist in the same package, the directory
(package) takes precedence.  Callers that do
``from claude_code.utils.hooks import ...`` therefore resolve to
``hooks/__init__.py``, not this file.  This file exists for documentation
purposes and as the authoritative list of the hooks public API.  The actual
re-exports live in ``hooks/__init__.py``.
"""
...
from claude_code.utils.hooks_execute import (
    ...
    # Duplicated in execute layer (use single source above, but keep importable)
    AggregatedHookResult as _AggregatedHookResult,  # already imported from types
    TOOL_HOOK_EXECUTION_TIMEOUT_MS as _TOOL_HOOK_EXECUTION_TIMEOUT_MS,  # same
