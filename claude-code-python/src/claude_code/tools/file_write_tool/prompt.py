"""FileWriteTool prompt constants. Ported from FileWriteTool/prompt.ts"""

from claude_code.tools.file_read_tool.prompt import FILE_READ_TOOL_NAME

FILE_WRITE_TOOL_NAME = "Write"
DESCRIPTION = "Write a file to the local filesystem."


def _get_pre_read_instruction() -> str:
    return (
        f"\n- If this is an existing file, you MUST use the {FILE_READ_TOOL_NAME} tool first "
        "to read the file's contents. This tool will fail if you did not read the file first."
    )


def get_write_tool_description() -> str:
    return (
        "Writes a file to the local filesystem.\n\n"
        "Usage:\n"
        f"- This tool will overwrite the existing file if there is one at the provided path."
        f"{_get_pre_read_instruction()}\n"
        "- Prefer the Edit tool for modifying existing files \u2014 it only sends the diff. "
        "Only use this tool to create new files or for complete rewrites.\n"
        "- NEVER create documentation files (*.md) or README files unless explicitly "
        "requested by the User.\n"
        "- Only use emojis if the user explicitly requests it. Avoid writing emojis to files "
        "unless asked."
    )
